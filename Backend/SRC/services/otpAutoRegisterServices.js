    ...
    try {
        ...
        if (users.length === 0) {
            ...
            const [newUser] = await mysql.query(
                'INSERT INTO users (name, phoneNo, role) VALUES (?,?,?)',
                ['User_' + phoneForDb, phoneForDb, 'customer']
            );
            const [created] = await mysql.query('SELECT * FROM users WHERE id = ?', [newUser.insertId]);
            user = created[0];
        } else {
            user = users[0];
        }
        ...
        return { Accesstoken, Refershtoken, user: { id: user.id, phoneNo: user.phoneNo, role: user.role } };
